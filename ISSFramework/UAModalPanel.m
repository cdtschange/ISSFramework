...
#define DEFAULT_HMARGIN				20.0f
#define DEFAULT_VMARGIN				20.0f
...
@synthesize closeBtnPosition,innerHMargin, innerVMargin, outerHMargin, outerVMargin, cornerRadius, borderWidth, borderColor, contentColor, shouldBounce;
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self != nil) {
		...
		outerHMargin = DEFAULT_HMARGIN;
        outerVMargin = DEFAULT_VMARGIN;
		innerHMargin = DEFAULT_HMARGIN;
		innerVMargin = DEFAULT_VMARGIN;
...
- (UIButton*)closeButton {
	if (!closeButton) {
		...
		[self.closeButton setImage:[UIImage imageNamed:IMG_UIMODALPANNEL_CLOSEBUTTON] forState:UIControlStateNormal];
...
- (CGRect)roundedRectFrame {
	...
	return CGRectMake(self.outerHMargin + self.frame.origin.x,
					  self.outerVMargin + self.frame.origin.y,
					  self.frame.size.width - 2*self.outerHMargin,
					  self.frame.size.height - 2*self.outerVMargin);
}
...
- (CGRect)contentViewFrame {
	CGRect rect = CGRectInset([self roundedRectFrame], self.innerHMargin, self.innerVMargin);
	return rect;
}
