...
- (NSString *) getDBPath {
    ...
    NSLog(@"dbpath:%@",documentsDirectory);
    return [documentsDirectory stringByAppendingPathComponent:self.dbName];
}

- (void)createTable:(NSString *)fields{
    char *errorMsg;
    
    NSString *sql=[NSString stringWithFormat:@"create table if not exists %@ (%@)",self.tableName,fields];
    if(sqlite3_exec(database, [sql UTF8String], NULL, NULL, &errorMsg)!=SQLITE_OK){
        sqlite3_close(database);
        NSLog(@"Database create table failed: %s",errorMsg);
    }
    NSLog(@"Database create table success");
}
...
- (void)dropTable{
    ...
    if(sqlite3_exec(database, [sql UTF8String], NULL, NULL, &errorMsg)!=SQLITE_OK){
        sqlite3_close(database);
        NSLog(@"Database drop table failed: %s",errorMsg);
    }
    NSLog(@"Database drop table success");
}
...
- (void) bindSQL:(const char *) cQuery arguments:(va_list)args {
    ...
    if ((param_count = sqlite3_bind_parameter_count(statement))) {
        for (int i = 0; i < param_count; i++) {
            ...
            } else if ([o isKindOfClass:[NSData class]]) { // blob
                sqlite3_bind_blob(statement, i + 1, [o bytes], [o length], SQLITE_TRANSIENT);
...
- (NSNumber *) insertRow:(NSDictionary *) record {
    ...
    id __unsafe_unretained keys[dictSize];  // not used, just a side-effect of getObjects:andKeys
    //id __unsafe_unretained values[dictSize];
    
    NSMutableData *data=[NSMutableData dataWithLength:sizeof(id) * dictSize];
    
    [record getObjects:( __unsafe_unretained id *)data.mutableBytes andKeys:keys];    // convenient for the C array
    ...
    NSLog(@"%@",query);
    [self bindSQL:[query UTF8String] arguments:(va_list)data.mutableBytes];
    sqlite3_step(statement);
    sqlite3_reset(statement);
    if(sqlite3_finalize(statement) == SQLITE_OK) {
...
- (void) updateRow:(NSDictionary *) record:(NSNumber *) rowID {
    ...
    [record getObjects:( __unsafe_unretained id *)data.mutableBytes andKeys:keys];    // 
...
- (NSDictionary *) getRow: (NSNumber *) rowID {
    ...
    NSDictionary *dict = [self getPreparedRow];
    sqlite3_reset(statement);
    return dict;
}

- (NSArray *) getRows: (NSString *) where {
    NSString * query = [NSString stringWithFormat:@"select * from %@ %@", tableName, where];
    [self prepareQuery:query];
    NSMutableArray *array=[[NSMutableArray alloc] init];
    NSDictionary *dict = [self getPreparedRow];
    while (dict!=nil) {
        [array addObject:dict];
        dict = [self getPreparedRow];
    }
    //sqlite3_reset(statement);
    return array;
}
