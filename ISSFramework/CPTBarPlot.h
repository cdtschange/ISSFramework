...
/**	@brief (Optional) fill new context for the given bar plot bar.
 *	@param plot The scatter plot.
 *  @param context plot context
 *  @param basePoint the base point
 *  @param tipPoint the tip point
 *  @param barWidthLength the bar width
 *	@param index Index of touched point
 **/
-(void)barDrawForBarPlot:(CPTBarPlot *)plot drawContext:(CGContextRef)context 
               basePoint:(CGPoint)basePoint tipPoint:(CGPoint)tipPoint 
          barWidthLength:(CGFloat)barWidthLength recordIndex:(NSUInteger)index;
@end
